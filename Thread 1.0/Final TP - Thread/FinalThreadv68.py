...
#!!!! Had to only use one file for this project because Pythonista does
#not allow for running of more than one file.

#Information about what libraries I had access to and what functions were
#available in those libraries were found off the pythonista OMZ website
...
class MyScene (Scene):
    ...
    def setup(self):
        #all colors for lightMode Theme
        ...
        #if the hour is between 6PM and 6AM put into dark Mode
        theHour = int(strftime("%H", gmtime()))
    ...
    def draw(self):
        ...
        # text('Thread', font_name='Courier', font_size=16.0, x=self.centerX2,\
        ...
        attitudeVectors = motion.get_attitude()
        pitch = attitudeVectors[0]
        roll = attitudeVectors[1]
        yaw = attitudeVectors[2]
        ...
        #function state 4 is about getting back to the original location
            ...
            # # text('Destination:', font_name='Verdana', font_size=12.0, \
            #x=70, y=550, alignment=5)
            # # text(loc, font_name='Verdana', font_size=12.0, x=230, \
            #y=550, alignment=5)
            ...
            if (len(self.locationsLeft) > 1):
                ...
                #had to derive this part on my own
                if degreeAngle < 0:
