...
class MyScene (Scene):
    ...
    def setup(self):
        ...
        # self.photoLibrary = photos.create_album(str(time.time()))
    ...
    def draw(self):
        ...
        if self.imageMode == True:
            text("Image Available", font_name='Courier', font_size=16.0, x=self.centerX2, y=self.centerY2+self.radius+300, alignment=5)
...
#-----------END OF PATHSTATE---------------------------------------------------
    ...
    def touch_began(self, touch):
        ...
        #take photos and add to album
        elif x < self.size.w/2 + 30 and x > self.size.w/2 - 30 and y > 50 and y < 100 and self.MoreState == False and self.measuringOn == True:
            ...
            if imageree != None:
                ...
                try:
                    album = [a for a in photos.get_albums() if a.title == 'Thread'][0]
                except IndexError:
