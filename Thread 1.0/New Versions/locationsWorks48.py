...
#-----------Draw Path State---------------------------------------------------
        if self.pathState == True and len(self.locations) > 0:
            ...
            maxDiffX = (max(evenedLocsX) - min(evenedLocsX))
            maxDiffY = (max(evenedLocsY) - min(evenedLocsY))
            ...
            for i in range(1,len(newLocs)):
                xLoc1, yLoc1 = evenedLocsX[i-1], evenedLocsY[i-1]
                xLoc2, yLoc2 = evenedLocsX[i], evenedLocsY[i]
                stroke(((255/colorCount)*i)/255, 0, 0)
                fill(((255/colorCount)*i)/255, 0, 0)
                line((323.5/generalMaxDiff)*xLoc1+25,(178/generalMaxDiff)*yLoc1+yCenter, (323.5/generalMaxDiff)*xLoc2+25,(178/generalMaxDiff)*yLoc2+yCenter)
