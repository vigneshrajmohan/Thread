    ...
    def draw(self):
        ...
        if self.measuringOn == False and self.functionState == 0:
            ...
            if self.timerCount//10 % 3 == 0:
                text('Recording Journey.', font_name='Verdana', font_size=16.0, x=self.centerX2, y=self.centerY2+self.scale+150, alignment=5)
        ...
        #The return back..
            ...
            if (len(self.locationsLeft) > 1):
                ...
                if (len(self.locationsLeft) > 8):
                    ...
                    xNextAverage = (backPosX + nextPosX + secondPosX + thirdPosX)/4
                    yNextAverage = (backPosY + nextPosY + secondPosY + thirdPosY)/4


                elif (len(self.locationsLeft) > 3):
                    currPosX, currPosY = self.locationsLeft[-1]
                ...
                def pointMaker(self,roll,pitch, yaw):
                    ...
                    line(self.centerX2-trueDegreeCos*130 ,self.centerY-trueDegreeSin*130 ,self.centerX2+trueDegreeCos*130 ,self.centerY+trueDegreeSin*130 )
                    stroke(self.theme["compassStrokeNorth"])
                    for i in range(20):
                        stroke_weight(20-i)
                        line(self.centerX2+trueDegreeCos*(130+(i*2)) ,self.centerY+trueDegreeSin*(130+(i*2)) , self.centerX2 ,self.centerY )
                ...
                # if latLong in self.photoLocations and self.imageModeOpen == False:
                ...
                #     text("Landmark! Tap here to see!", font_name='Verdana', font_size=16.0, x=self.centerX2, y=150+25, alignment=5)
        ...
        if self.MoreState == True:
            ...
            ellipse(self.centerX2-self.scale,self.size.y-115,self.scale*2,self.scale*2)
            text('Theme', font_name='Verdana', font_size=12.0, x=self.centerX2, y=self.size.y-77, alignment=5)
...
#-----------END OF PATHSTATE---------------------------------------------------
    ...
    def touch_began(self, touch):
        ...
        if self.loopPrompt == True:
            ...
            elif x < self.size.w/2 + 30 and x > self.size.w/2 - 30 and y > self.size.y-95-40 and y < self.size.y-95+40:
                self.themeSwitch += 1
        ...
        elif self.measuringOn == True and self.functionState == 1 and len(self.locations) < 4:
            #sound.play_effect('arcade:Laser_1')
            self.needMore = True

        elif self.measuringOn == True and self.functionState == 1 and len(self.locations) > 3:
            #sound.play_effect('arcade:Laser_1')
            self.needMore = False
            self.measuringOn = False
            self.background_color = self.theme["backgroundColor"]
